...
def plot_Mean2DMSDsorDeff(traj, n1, n2, n3, dec1, dec2, datatype, filename, limit1, limit2):
    ...
    ax.plot(time[1][:], MMSD[:, 0], linewidth=8, label='3D')
    ax.plot(time[1][:], MMSD[:, 1], linewidth=8, label='2D xy')
    ax.plot(time[1][:], MMSD[:, 2], linewidth=8, label='2D xz')
    ax.plot(time[1][:], MMSD[:, 3], linewidth=8, label='2D yz')
