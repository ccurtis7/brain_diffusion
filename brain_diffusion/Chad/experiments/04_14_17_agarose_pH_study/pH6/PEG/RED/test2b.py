...
for num in range(1, totvids + 1):
    conversion[num] = (0.16, 9.9, 1)
    trajectory[num] = np.genfromtxt('./Traj_RED_PEG_in_agarose_pH6_2_{}.tif.csv'.format(num), delimiter =",")
...
exists = os.path.isfile('pM1xb_{}.csv'.format(rank))

if exists == False:
    last_size = 1
    with open('pM1xb_{}.csv'.format(rank), "wb") as f_handle:
        pass
    with open('pM1yb_{}.csv'.format(rank), "wb") as f_handle:
        pass
    with open('pM2xyb_{}.csv'.format(rank), "wb") as f_handle:
        pass
else:
    with open('pM1xb_{}.csv'.format(rank), "rb") as f_handle:
